...
Embed（title「CTF募集一覧 ({status ラベル})」）で応答。0 件なら description は「該当する募集はありません。」。それ以外は先頭行「{n}件を表示しています。」（n は DB から取得した件数。最大 20 件 = `list_campaigns` の limit）に続けて、campaign ごとの以下のブロックを空行区切りで列挙する（`created_at_unix` 降順）:
...
各タスクの対象取得は期日（`start_at_unix` / `end_at_unix` / `archive_at_unix`）の古い順に 20 件/分ずつ。処理しきれなかった分は翌分に最古から再試行される。
...
- `require_role_channel`: **CTF募集カテゴリ内**の名前が `role` に完全一致するテキストチャンネルを解決する（複数あれば最初の 1 件）。失敗時「#role チャンネルが見つかりません。」

### チャンネル名の正規化

`normalize_channel_name`: times の正規化（`docs/features/times.md`）と同一の変換（lowercase → `[^a-z0-9\-]` を `-` に置換 → 連続 `-` を 1 つに → 先頭末尾 `-` 除去）に、結果が空なら `"ctf"` にフォールバックする点だけを加えたもの。フォールバック後に 100 文字で切り詰める。
...
参加メンバーはロール保持者（member cache。`Intents.members` 前提）で、列挙順は保証しない。0 人の場合はヘッダー行のみ送る（スナップショットは「参加メンバー (0人):」まで）。ヘッダー行は `AllowedMentions.none()`、メンションチャンクは users のみ許可（everyone・roles 拒否）で送る（`docs/core.md` のメンション方針）。
...
- `ctf_name` は `COLLATE NOCASE` で大文字小文字無視（case folding は ASCII の 26 文字のみ。非 ASCII の名前は大文字小文字が区別される）
...
| 環境変数 | 必須 | 説明 |
|---|---|---|
| `CTF_TEAM_CATEGORY_ID` | Yes | `#role`（募集メッセージの投稿先）・discussion・voice を配置するカテゴリ |
