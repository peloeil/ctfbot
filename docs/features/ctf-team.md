...
| ファイル | 責務 |
|---|---|
| `models.py` | `Campaign`, `CampaignDraft`, `CampaignStatus` データモデル |
| `campaign.py` | バリデーション・ビジネスロジック（Discord 非依存） |
| `discord_ops.py` | Discord リソース操作（チャンネル・ロール・メッセージ） |
...
2. バリデーション（`campaign.validate_and_build_draft`）
