...
通知を DB 遷移後・claim 条件付きに置くことで、手順 1〜3 の失敗によるリトライや `/ctfteam archive` と毎分ループの競合で通知が重複しない（close のスナップショットと同じパターン）。
...
**DB insert 前（手順 1〜4）の失敗**は作成済み Discord リソースをロールバック（`discord_ops.cleanup_resources`。DB は対象外）:
