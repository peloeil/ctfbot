...
Embed（title「CTF募集一覧 ({status ラベル})」）で応答。0 件なら description は「該当する募集はありません。」。それ以外は先頭行「{n}件を表示しています。」に続けて、campaign ごとの以下のブロックを空行区切りで列挙する（`created_at_unix` 降順、最大 20 件 = `list_campaigns` の limit）:
...
description の 4096 文字上限を超えるブロック以降は載せず、「他 {n} 件は省略しています。」を追加して打ち切る。
...
- ロールを付与し、discussion チャンネルに参加通知を投稿（付与失敗は warning ログのみで中断）
