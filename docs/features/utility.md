...
`bot.tree.get_commands()` で全コマンド取得。Group コマンドは展開して `/group subcommand — description` 形式で列挙し、ソートして応答。

既知の制限: 応答の分割・切り詰めは行わないため、コマンド数の増加で Discord の 2000 文字上限に達すると送信に失敗する。

### `/perms [channel]`

bot が持つ権限を `✅ {name}` / `❌ {name}` の行区切りで表示する。`channel`（`discord.TextChannel` のみ指定可）省略時は実行チャンネルを対象とする。
