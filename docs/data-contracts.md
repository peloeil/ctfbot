...
`SolveRecord` と、その週次集計である `WeeklySolveSummary`、Daily 問題を表す `DailyChallenge` を定義する。`DailyChallenge` は title・url・author・categories・difficulty・description・attachment_urls・starts_at・ends_at を保持する。`solved_at`・`starts_at`・`ends_at` は `Settings.tzinfo` へ変換済みの aware datetime。`weekly_solves` のキーは username。取得に失敗したユーザーは `weekly_solves` に含めず `failed_users` にのみ入れる。
