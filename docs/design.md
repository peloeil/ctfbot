...
- 実装ファイルを正本として参照しない（正本は常に docs/ 側。どの文書が何の正本かは CLAUDE.md・AGENTS.md の参照表による）。例外は、それ自体が実行される設定ファイル（`.github/workflows/ci.yml`・`pyproject.toml` 等）。これらは実物が正本であり、docs には全文を複製せず、保証・非目標・設計理由を書く
- 型宣言（dataclass のフィールド一覧）は docs に複製せず、型の役割・invariant・状態遷移だけを書いて実装を参照する。DB スキーマ（DDL・migration）は例外で、過去の migration SQL と列順が実装からしか復元できないため docs/data-contracts.md を正本とする
