...
pub async fn echo_command(command: &CommandInteraction, ctx: &Context) {
    ...
    let value = message_option
        .value
        .as_str()
        .expect("Expected string value for message option");
...
pub async fn pin_unpin_command(command: &CommandInteraction, ctx: &Context) {
    ...
    let data =
        CreateInteractionResponseMessage::new().content(format!("{} message", command.data.name));
    let builder = CreateInteractionResponse::Message(data);
    if let Err(why) = command.create_response(&ctx.http, builder).await {
        println!("Cannot respond to slash command: {why}");
    }
