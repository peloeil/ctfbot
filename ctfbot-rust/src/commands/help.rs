...
pub async fn help_command(command: &CommandInteraction, ctx: &Context) {
    ...
    help_message
        .push_str("`/unpin <message_link>`: 指定されたメッセージのピン留めを解除します。\n\n");
    ...
    help_message
        .push_str("`/show_alpaca_score`: 追跡中のAlpacaHackユーザーのスコアを表示します。\n\n");
...
pub fn register_help_command(commands: &mut Vec<CreateCommand>) {
    commands.push(CreateCommand::new("help").description("Show all available commands"));
}
